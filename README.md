...
I thought it might be cool and very bubble'y to add a "popping" effect to the animation. After all, real bubbles don't just grow indefinitely. I added a conditional statement to "bubbles_1.py" which deleted bubbles which had exceeded a certain radius (r>s/1.5) where *s* is the canvas width. This is kind of cool, but I've kept it to just the first GIF. I think it often looks better if the bubbles don't pop, as unrealistic as that might be. Maybe it would help if the popping animation was a quick, but smooth fadeout rather than an abrupt deletion, though I am not super eager to labor over that at this point in time.
